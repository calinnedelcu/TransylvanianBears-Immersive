...
function TrackedSubjects({
  ...
  const geometries = useMemo(() => ({
    box: new THREE.BoxGeometry(1, 1, 1),
    person: new THREE.CapsuleGeometry(0.26, 1.35, 5, 10),
  }), []);
  const materials = useMemo(() => ({
    solid: createSemanticMaterial('#ffffff', '#cf6554', 5),
    person: createSemanticMaterial('#bab9b0', '#72d9d6', 5),
  ...
  useLayoutEffect(() => {
    setInstanceTransform(solidRef.current, 0, scratch, -2.15, 0.38, 0, 1.5, 0.66, 2.25);
    setInstanceTransform(solidRef.current, 1, scratch, 2.8, 1.2, -0.35, 1.9, 2.4, 1.7);
    ...
    setInstanceTransform(personRef.current, 0, scratch, 0.15, 1.15, 0.4, 1, 1, 1);
    setInstanceTransform(boundaryRef.current, 0, scratch, -2.15, 0.5, 0, 1.8, 1.2, 2.55);
    setInstanceTransform(boundaryRef.current, 1, scratch, 0.15, 1.2, 0.4, 0.9, 2.7, 0.9);
    setInstanceTransform(boundaryRef.current, 2, scratch, 2.8, 1.24, -0.35, 2.2, 2.75, 2);
    [solidRef.current, personRef.current, boundaryRef.current].forEach(markInstanceMatrixDirty);
  ...
  useFrame(({ gl, size }) => {
    const lensPresence = smooth(range(progressRef.current, 0.13, 0.16))
      * (1 - smooth(range(progressRef.current, 0.285, 0.318)));
    updateSemanticMaterials(
      [materials.solid, materials.person],
      lensMode,
      lensPointerRef,
      size.width,
      size.height,
  ...
  return (
    <group position={[0, 0, -32]}>
      ...
      <instancedMesh ref={personRef} args={[geometries.person, materials.person, 1]} frustumCulled={false} />
      <instancedMesh
        ref={boundaryRef}
        args={[geometries.box, materials.boundary, 3]}
...
function NexusCity({
  ...
  return (
    <group ref={rootRef} position={[0, 0, -7]}>
      ...
      <TrackedSubjects progressRef={progressRef} lensMode={lensMode} lensPointerRef={lensPointerRef} />
    </group>
  );
}
